...
context "Master Worker in general should" do
  ...
  it "read data according to binary protocol and recreate objects" do
    sync_request = { :type=>:sync_invoke, 
                     :arg=>"boy", 
                     :worker=>:foo_worker, 
                     :worker_method=>"barbar" }
    @master_worker.expects(:method_invoke).with(sync_request).returns(nil)
    @master_worker.receive_data(packet_dump(sync_request))
  end

  it "ignore errors while recreating object" do
    sync_request = {
  ...
  it "should route async requests" do
    b = {
  ...
  it "should route sync requests and return results" do
    a = { :type=>:sync_invoke, 
          :arg=>"boy", 
          :worker=>:foo_worker, 
          :worker_method=>"barbar"}
  ...
  it "should route start worker requests" do
  ...
  it "should run delete worker requests itself" do
  ...
  it "should route worker info requests" do
  ...
  it "should route all_worker_info requests" do
  ...
  it "should route worker result requests" do
  ...
  it "should remove the worker from list if error while fetching results" do
    c = {:type=>:get_result, :worker=>:foo_worker, :job_key=>:start_message}
  ...
  it "should ignore generic exceptions while fetching results" do
    c = {:type=>:get_result, :worker=>:foo_worker, :job_key=>:start_message}
